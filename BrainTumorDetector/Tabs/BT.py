...
def app():
    ...
    st.subheader("Brain Tumor Predictor")
    ...
    if x:
        with st.spinner("Predicting..."):
            y,conf = imagerec.imagerecognise(uploaded_file,"BrainTumorDetector/Models/BrainTumuorModel.h5","BrainTumorDetector/Models/BrainTumuorLabels.txt")
        if y.strip() == "Safe":
            components.html(
                """
                <style>
        ...
        '''testds = models.testds

        model = models.load_model()
        test_loss, test_acc = model.evaluate(testds)
        st.sidebar.warning("Accuracy : " +test_acc + " %")
'''